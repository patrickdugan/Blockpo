...
def clearing_operator(M_file, obj_trk, amount_trd_sum, path_complex_two, idx_i, index_init, addrs_trk_arg, amount_trd_begining, index_long_short):
    ...
    average_incr.append([obj_trk.addrs_src, obj_trk.lives_src, obj_trk.status_src, obj_trk.addrs_trk,
                         obj_trk.lives_trk, obj_trk.status_trk, obj_trk.amount_trd, obj_trk.matched_price, index_init])

    howmany_netted = 0
    numberof_lives_contracts_byaddress = 0

    howmany_netted, average_incr = howmany_netted_events_and_vectorwithincrs(
        howmany_netted, index_init, M_file, addrs_trk_arg, index_long_short, average_incr)
        ...
        if addrs_trk_arg in N_filei and bool_netted_status:
            ...
            if len(average_incr) > 1:
                ...
                if balance_incr <= balance_increasing:
                    ...
                    traded_position_incr = average_posincreased(
                    ...
                    path_complex_two = long_short_incr_path(
                        traded_position_incr, obj_trk_inloop, path_complex_two, index_init)
                    ...
                    if path_complex_two[-1]['status_src'] in globales.open_incr_long_short:
                        print(
                            "Can be", path_complex_two[-1]['addrs_src'], "a new source??")
                        ...
                        M_file, idx_i, path_complex_incrs = clearing_operator(
                            M_file, obj_trk_incr, amount_trd_sum_new, path_complex_incrs, idx_i, i, addrs_trk_incr, obj_trk_incr.amount_trd, idx_long_or_short)
            ...
            if d_amounts > 0:
                ...
                M_file = update_lasttwo_columns(
                    obj_trk_inloop, N_filei, M_file, i)
                print("\nRow update:\n", M_file[:][i])
                print("\nOpened contrats: ", obj_trk.amount_trd,
                ...
                if bool_src:
                    ...
                    print("\nLooking for New Path for: ",
                          obj_trk_inloop.addrs_src, "!!\n")
                    M_file, idx_i, path_complex_first = clearing_operator(
                        M_file, obj_trk_inloop, amount_trd_sum_new, path_complex_first, idx_i, i, obj_trk_inloop.addrs_src, obj_trk_inloop.amount_trd, idx_long_or_short)
                continue
                ...
                print("Opened contrats: ", obj_trk.amount_trd,
                      " < Sum amounts traded: ", amount_trd_sum, "diff_r: ", diff_r, "\n")

                print("amount_trdi", obj_trk_inloop.amount_trd, "lives_trki",
                      obj_trk_inloop.lives_trk, "status_trki: ", obj_trk_inloop.status_trk)
                x_src = abs(obj_trk_inloop.amount_trd - obj_trk_inloop.lives_src) if "Long" in str(
                    obj_trk_inloop.status_src) else -abs(obj_trk_inloop.amount_trd - obj_trk_inloop.lives_src)
                y_trk = abs(obj_trk_inloop.amount_trd + obj_trk_inloop.lives_trk) if "Long" in str(
                    obj_trk_inloop.status_trk) else -abs(obj_trk_inloop.amount_trd - obj_trk_inloop.lives_trk)
                print("x_src: ", x_src, "y_trk", y_trk)

                path_complex_ele_value_thr = [obj_trk_inloop.addrs_src, obj_trk_inloop.addrs_trk, obj_trk_inloop.status_src,
                                              obj_trk_inloop.status_trk, obj_trk.matched_price, obj_trk_inloop.matched_price, diff_r, 0]
                path_complex_ele_thr = OrderedDict(
                    zip(globales.key_path, path_complex_ele_value_thr))

                print("(d_amounts < 0) path_complex_ele_thr: \n\n",
                      path_complex_ele_thr)

                path_complex_two.append(path_complex_ele_thr)
                print("\n\n'iteration: ", i, "|addrs_trk: ", addrs_trk_arg, "|status_trki: ", obj_trk_inloop.status_trk,  "|amount_trdi: ",
                      obj_trk_inloop.amount_trd, "|addrs_srci: ", obj_trk_inloop.addrs_src, "|status_srci: ", obj_trk_inloop.status_src, "\n")
                print("\n------------------------------------------------------\n")
                idx_i.append(i)

                amount_new = abs(diff_r - obj_trk_inloop.amount_trd)
                new_row = np.array([obj_trk_inloop.addrs_src, obj_trk_inloop.status_src, obj_trk_inloop.lives_src, obj_trk_inloop.addrs_trk, obj_trk_inloop.status_trk,
                                    obj_trk_inloop.lives_trk, amount_new, obj_trk_inloop.matched_price, obj_trk_inloop.lives_src, obj_trk_inloop.lives_trk])
                ...
                if bool_src:
                    ...
                    print("\nLooking for New Path for: ",
                          obj_trk_inloop.addrs_src, "!!\n")
                    M_file, idx_i, path_complex_secnd = clearing_operator(
                        M_file, obj_trk_inloop, amount_trd_sum_new, path_complex_secnd, idx_i, i, obj_trk_inloop.addrs_src, obj_trk_inloop.amount_trd, idx_long_or_short)
                else:
                    break
                ...
                if bool_src:
                    ...
                    M_file, idx_i, path_complex_third = clearing_operator(
                        M_file, obj_trk_inloop, amount_trd_sum_new, path_complex_third, idx_i, i, obj_trk_inloop.addrs_src, obj_trk_inloop.amount_trd, idx_long_or_short)
                break
...
def first_single_path(m):
    ...
    if len(m) == 1:
        ...
        single_path_value_begin = [m[0][0], m[0][3],
                                   m[0][1], m[0][4], m[0][7], 0, m[0][6], m[0][6]]
...
def long_short_incr_path(traded_pos_incr, obj_trk_inloop, path_complex_two, index_init):
    ...
    for trade_amount_incr in traded_pos_incr:
        print(
            "Checking trade_amount_incr[6] >= index_init: ", trade_amount_incr[6], index_init)
        if trade_amount_incr[6] >= index_init:
            entry_price_increase = trade_amount_incr[1]
            path_complex_value_ele_two = [obj_trk_inloop.addrs_src, obj_trk_inloop.addrs_trk, obj_trk_inloop.status_src,
                                          obj_trk_inloop.status_trk, entry_price_increase, obj_trk_inloop.matched_price, trade_amount_incr[0], 0]
            path_complex_ele_two = OrderedDict(
                zip(globales.key_path, path_complex_value_ele_two))
            path_complex_two.append(path_complex_ele_two)
        else:
...
def howmany_netted_events_and_vectorwithincrs(howmany_netted, index_init, M_file, addrs_trk_arg, index_long_short, average_incr):
    ...
    for i in range(index_init+1, len(M_file)):
        ...
        obj_trk_inloop = status_amounts_inloop(N_filei, addrs_trk_arg)
        average_incr, i = vector_pos_incr(
            addrs_trk_arg, obj_trk_inloop, N_filei, average_incr, i, index_init, M_file, index_long_short)
        bool_netted_status = boolean_for_netted_status(obj_trk_inloop)
    ...
    average_incr = reverseiterator_incr_pos(
        index_init, addrs_trk_arg, average_incr)
