...
with open('graphInfoFourth.txt') as file:
    M_json = [[int(digit) if digit.isdigit() else digit for digit in line.split()] for line in file]
...
for k in range(len(M_json)):
    X = [M_json[k][0], M_json[k][2], '%d' %M_json[k][1], '{middle:{scaleFactor:0.5},from:true},smooth:{type: "curvedCW", roundness: %0.2f}' %random.uniform(0, 1)]
