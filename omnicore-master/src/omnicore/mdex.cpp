    ...
    for (cd_PricesMap::iterator priceIt = ppriceMap->begin(); priceIt != ppriceMap->end(); ++priceIt) { // check all prices
        ...
        while (offerIt != pofferSet->end()) { // Specific price, check all properties
            ...
            if( (getMPbalance(pnew->getAddr(), pnew->getProperty(), POSSITIVE_BALANCE) > 0 ) && (getMPbalance(pnew->getAddr(), pnew->getProperty(), NEGATIVE_BALANCE) == 0 ) ) {
             ...
             // if difference are negative, the closed contracts are zero
            ...
            // PrintToConsole("Market Price: %d, countClosedBuyer: %d, countClosedSeller: %d, lives_taker: %d, lives_maker: %d, Match Price: %d\n", marketPrice, countClosedBuyer, countClosedSeller, lives_taker, lives_maker, pold->getEffectivePrice());
            if ( Status_s != "None") {
           ...
           /////// HAY PROBLEMAS CON EL TINY FORMAT, POR ESO HE COMENTADO ESTAS LINEAS !!!
