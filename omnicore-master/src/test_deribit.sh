...
# adding  price and sell/buy option
Action=( $(cut -d ',' -f2 sellbuy.csv ) )
Prices=( $(cut -d ',' -f2 deribit.csv ) )
for (( i=1; i<=${N}; i++ ))
do
       echo ${Prices[$i]}
done
...
##################################################################
printf "\n________________________________________\n"
printf "Creating an Divisible Token USDT:\n"   # TODO: see the indivisible/divisible troubles (amountToReserve < nBalance) in logicMath_Contr$
...
##################################################################
# Price: 11000-11200, Amount Buy 80 Addrs: 100-5000 | Amount Sell 100 Addrs: 100-5000, Price: 11100-11300 | Amount Buy 100 Addrs: 100-5000, P$


#TRATradeBuy=()
for (( i=1; i<=${N}/2+30; i++ ))
do      
        printf "Market Price of contract now in deribit:\n"
           
        printf ${Prices[$i]}
        echo "${i},${Prices[$i]}" >> prices.csv
        printf "\n////////////////////////////////////////\n"
...
#       #TRATradeBuy[$i]=$(
        ./omnicore-cli -datadir=$DATADIR --regtest omni_tradecontract ${ADDRess[$i]} ${CONTRACT} ${AMOUNT} 1 1 ${Prices[$i]} ${ACTION}
        ./omnicore-cli -datadir=$DATADIR --regtest generate 1
