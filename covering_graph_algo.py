...
for j in range(len(N_file)):
    ...
    if bool_track_long and bool_track_short:
        ...
        N_file, idx_i, path_complex_two_long = clearing_operator(N_file, obj_long_trk, amount_trd_sum, path_complex_two_long, idx_i, j, obj_long_trk.addrs_trk, obj_long_trk.amount_trd, 0, diff_trdamount)
    ...
    # Joining every all the paths in "path_complex_main" #
    path_complex_main = []
    ...
    ################################################################
    # Function to check Zero Netted by Path #
    contracts_opened = 0
    contracts_closed = 0
    contracts_lives = 0
