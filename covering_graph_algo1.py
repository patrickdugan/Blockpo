...
for j in range(len(N_file)):
    ...
    if bool_track_long and bool_track_short:
        ...
        sum_oflives, exit_price_desired = checking_pathcontaining_livesnonzero(path_complex_main)   

        dict_lives_prices[sum_oflives] = exit_price_desired

    path_complex_two_matrix.append(path_complex_main)

pnl_total = total_pnl(path_complex_two_matrix)
print("\nTotal PNL = ", pnl_total)
print("\ndict_lives_prices:\n", np.array(dict_lives_prices))
