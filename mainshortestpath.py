            ...
            for i in xrange(1, len(M_file)):
                ...
                if addrs_trk in str(N_filei) and status_trki == "LongPosNetted":
                    ...
                    if amount_trd_sum <= amount_trd:
                        ...
                        diff_later = int(amount_trd) - \
                            int(amount_trd_sum_later)
                        ...
                        diff_before = int(amount_trd) - \
                            int(amount_trd_sum_before)
                        path_complex_ele_thr.append(N_filei[0])
                        path_complex_ele_thr.append(abs(diff_before))
                        path_complex_ele_thr.append(N_filei[3])

                        print "amount_trd_sum_before: ", amount_trd_sum_before, "amount_trd_sum_later: ", amount_trd_sum_later, "amount_trd: ", N_filei[
                            6]
                        idx_i.append(i)

                        amount_new = abs(diff_before - N_filei[6])
                        M_file.append([N_filei[0], N_filei[1], a,
                                       N_filei[3], N_filei[4], b, amount_new])

            if path_complex_ele_thr != [] and path_complex_two != []:

                print "Opened contrats: ", amount_trd, " < Total traded contracts: ", amount_trd_sum, " -> Amount traded this contract", abs(
                    amount_trd_sum_later - amount_trd_sum_before), "\n"
                print "Complex path:\n", path_complex_two
