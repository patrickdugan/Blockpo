...
///////////////////////////////////////////
/** New things for Contracts */
uint256 GetConsensusHashContractDex()
{
    // allocate and init a SHA256_CTX
    SHA256_CTX shaCtx;
    SHA256_Init(&shaCtx);
    ...
    // Balances - loop through the tally map, updating the sha context with the data from each balance and tally type
    // Placeholders:  "address|propertyid|balance|selloffer_reserve|accept_reserve|contractdex_reserve"
    // Sort alphabetically first
    std::map<std::string, CMPTally> tallyMapSorted;
    for (std::unordered_map<string, CMPTally>::iterator uoit = mp_tally_map.begin(); uoit != mp_tally_map.end(); ++uoit) {
        tallyMapSorted.insert(std::make_pair(uoit->first,uoit->second));
...
////////////////////////////////////////////////
/** New things for Contracts */
uint256 GetContractDexHash(const uint32_t propertyId)
{
    SHA256_CTX shaCtx;
    SHA256_Init(&shaCtx);
    ...
    uint256 contractdexHash;
    SHA256_Final((unsigned char*)&contractdexHash, &shaCtx);

    return contractdexHash;
}
/////////////////////////////////////////////////
